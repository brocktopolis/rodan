...
void ofEasyCam::manualRotate(ofVec3f rot){
	ofRectangle viewport = getViewport(this->viewport);
	xRot = 0;
	yRot = 0;
	zRot = 0;
	...
	ofVec2f center(viewport.width/2, viewport.height/2);
	//zRot = -vFlip * ofVec2f(endMouse.x - viewport.x - center.x, endMouse.y - viewport.y - center.y).angle(endMouse - ofVec2f(viewport.x, viewport.y) - center);
	xRot = vFlip * -rot.x * rotationFactor;
	yRot = -rot.y * rotationFactor;
}
